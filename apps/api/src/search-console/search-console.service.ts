...
export class SearchConsoleService {
  ...
  async completeCallback(state: string, code: string, error?: string) {
    ...
    const existing = await prisma.searchConsoleConnection.findUnique({ where: { projectId_userId_property: { projectId: pending.projectId, userId: pending.userId, property: pending.property } } });
