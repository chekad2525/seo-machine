...
export default function AppHeader({ active, identity, connected = false }: { active: ActivePage; identity?: string | null; connected?: boolean }) {
  return <header className="app-product-header app-command-header">
    ...
    <nav className="dashboard-links" aria-label="ناوبری اصلی">
      ...
      <Link className={active === 'analytics' ? 'active' : ''} href="/analytics">تحلیل‌ها</Link>
    ...
    <div className="header-tools">
      <details className="mega-menu">
        ...
        <div className="mega-panel">
          <div className="mega-intro"><span>SEO MACHINE / COMMAND</span><h2>از سیگنال جست‌وجو<br/>به تصمیم برسید.</h2><p>ابزارهای روزانه پایش، تحلیل و اقدام در یک مسیر.</p><div className={connected ? 'mega-health ready' : 'mega-health'}><i/>{connected ? 'داده‌های Search Console متصل است' : 'اتصال Search Console نیاز به بررسی دارد'}</div></div>
          <div className="mega-groups">
            ...
            <section><p>تحلیل و اقدام</p><Link href="/analytics"><span>۰۳</span><div><b>فرصت‌های رشد</b><small>صفحات و عبارت‌های اولویت‌دار</small></div><i>←</i></Link><Link href="/dashboard#connection"><span>۰۴</span><div><b>اتصال و دریافت داده</b><small>مدیریت GSC و همگام‌سازی</small></div><i>←</i></Link></section>
          </div>
          <div className="mega-foot"><span>داده‌های رتبه با تأخیر معمول ۲–۳ روز Search Console</span><Link href="/keywords">بازکردن رهگیر <b>←</b></Link></div>
        </div>
      </details>
