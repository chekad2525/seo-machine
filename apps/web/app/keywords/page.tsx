...
export default async function KeywordsPage() {
  ...
  const data = project ? await internalJson<{ range: { startDate: string; endDate: string }; freshnessNote: string; exactRankNote: string; exactRankReady: boolean; gscConnected: boolean; keywords: TrackedKeyword[]; suggestions: KeywordSuggestion[] }>(`/api/v1/integrations/google-search-console/keyword-tracking?projectId=${encodeURIComponent(project.id)}`, session.user.id) : null;
