...
export const metadata = privatePageMetadata('رهگیری رتبه کلمات کلیدی', 'گزارش خصوصی جایگاه واقعی کلمات کلیدی، تغییرات رتبه و اقدام‌های پیشنهادی برای پروژه شما.');
...
export default async function KeywordsPage() {
  ...
  const report = project ? await internalJson<{ settings: TrackingSettings; capabilities: { mobile: boolean }; summary: TrackingSummary; rankDepth: number; exactRankNote: string; exactRankReady: boolean; keywords: TrackedKeyword[] }>(`/api/v1/keyword-tracking?projectId=${encodeURIComponent(project.id)}`, session.user.id) : { data: null, error: null };
  const data = report.data;
  const identity = session.user.email ?? session.user.name;
  return <main className="dashboard-shell app-dashboard keyword-page" dir="rtl"><a className="skip-link" href="#keyword-content">پرش به رهگیری کلمات</a><AppHeader active="keywords" identity={identity}/><div className="formal-dashboard" id="keyword-content"><section className="keyword-hero"><div><p className="app-overline">رادار عبارت‌های کلیدی</p><h1>جایگاه را ببینید؛<br/><em>اقدام بعدی را پیدا کنید.</em></h1><p>جایگاه مشاهده‌شده دامنه را با موقعیت، زبان و دستگاه ثابت بررسی کنید و برای هر عبارت یک تصمیم روشن سئو داشته باشید.</p><div className="keyword-hero-badges"><span>نتایج زنده گوگل</span><span>مستقل از Search Console</span></div></div><div className="keyword-radar" aria-hidden="true"><i/><i/><i/><span><b>{data?.keywords.length ?? 0}</b><small>کلمه زیر نظر</small></span></div></section>{onboarding.error ? <section className="analysis-unavailable keyword-service-error"><p className="app-overline">خطای اتصال سرویس</p><h2>اطلاعات پروژه دریافت نشد.</h2><p>{onboarding.error}</p><Link className="app-submit inline" href="/keywords">تلاش دوباره <span>←</span></Link></section> : !project ? <section className="analysis-unavailable keyword-project-empty"><p className="app-overline">شروع رهگیری مستقل</p><h2>اولین پروژه رهگیری را بسازید.</h2><p>فقط نام پروژه و دامنه لازم است. اتصال Search Console برای این بخش ضروری نیست.</p><KeywordProjectSetup/></section> : report.error || !data ? <section className="analysis-unavailable keyword-service-error"><p className="app-overline">پروژه: {project.domain}</p><h2>گزارش جایگاه دریافت نشد.</h2><p>{report.error ?? 'پاسخ گزارش قابل استفاده نبود.'}</p><Link className="app-submit inline" href="/keywords">تلاش دوباره <span>←</span></Link></section> : <KeywordTracker projectId={project.id} initialKeywords={data.keywords} settings={data.settings} capabilities={data.capabilities} summary={data.summary} rankDepth={data.rankDepth} exactRankNote={data.exactRankNote} exactRankReady={data.exactRankReady}/>}</div></main>;
}
