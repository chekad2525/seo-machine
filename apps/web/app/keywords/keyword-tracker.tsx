...
export default function KeywordTracker({ projectId, initialKeywords, settings, capabilities, summary, rankDepth, exactRankNote, exactRankReady }: { projectId: string; initialKeywords: TrackedKeyword[]; settings: TrackingSettings; capabilities: { mobile: boolean }; summary: TrackingSummary; rankDepth: number; exactRankNote: string; exactRankReady: boolean }) {
  ...
  useEffect(() => setPage(1), [priority, rankBand, search, status]);
  ...
  return <>
    ...
    <section className="keyword-table-card">
      ...
      {rows.length ? <div className="keyword-table-scroll"><table className="keyword-table strategy-table"><thead><tr><th>کلمه کلیدی</th><th>جایگاه مشاهده‌شده</th><th>تغییر</th><th>تاریخچه</th><th>صفحه رتبه‌گرفته</th><th>صفحه هدف</th><th>اقدام پیشنهادی</th><th><span className="sr-only">عملیات</span></th></tr></thead><tbody>{pageRows.map((item) => <tr key={item.id}><td><b>{item.query}</b><small>Google · {settings.device === 'mobile' ? 'موبایل' : 'دسکتاپ'} · {item.exact?.provider ?? 'در انتظار'}</small></td><td><div className="exact-rank" title={item.exactError ?? ''}>{item.exact ? item.exact.rank === null ? <><span className="rank-not-found">یافت نشد</span><small>در {faNumber.format(rankDepth)} نتیجه</small></> : <><strong>{faNumber.format(item.exact.rank)}</strong><small>{new Date(item.exact.checkedAt).toLocaleDateString('fa-IR')}</small></> : item.exactStatus === 'PENDING' ? <span className="exact-pending">در صف</span> : item.exactStatus === 'FAILED' ? <span className="exact-failed">خطا</span> : <span className="trend-empty">بررسی نشده</span>}</div></td><td>{item.exact?.change === null || item.exact?.change === undefined ? <span className="rank-change neutral">—</span> : <span className={`rank-change ${item.exact.change < 0 ? 'up' : item.exact.change > 0 ? 'down' : 'neutral'}`}>{item.exact.change < 0 ? '↑' : item.exact.change > 0 ? '↓' : '—'} {item.exact.change ? faNumber.format(Math.abs(item.exact.change)) : ''}</span>}</td><td><Trend points={item.exactHistory}/></td><td className="tracked-url">{item.exact?.resultUrl ? <a href={item.exact.resultUrl} target="_blank" rel="noreferrer" title={item.exact.resultUrl}>{urlPath(item.exact.resultUrl)}</a> : '—'}</td><td className="tracked-url">{item.targetPage ? <a href={item.targetPage} target="_blank" rel="noreferrer" title={item.targetPage}>{urlPath(item.targetPage)}</a> : 'تعیین نشده'}</td><td><div className="strategy-action"><span className={`priority ${item.action.priority}`}>{priorityLabels[item.action.priority]}</span><div><b>{item.action.title}</b><small>{item.action.detail}</small></div></div></td><td><button className="remove-keyword" type="button" disabled={Boolean(busy)} onClick={() => void run(item.id, { action: 'remove', id: item.id }, 'کلمه از فهرست حذف شد.')}>{busy === item.id ? '…' : 'حذف'}</button></td></tr>)}</tbody></table></div> : <div className="keyword-empty"><span>⌁</span><h3>{initialKeywords.length ? 'نتیجه‌ای با این فیلتر پیدا نشد.' : 'هنوز کلمه‌ای زیر نظر نیست.'}</h3><p>{initialKeywords.length ? 'فیلترها را تغییر دهید.' : 'فایل Excel یا CSV وارد کنید یا اولین کلمه را دستی اضافه کنید.'}</p></div>}
      <Pagination page={safePage} pageCount={pageCount} totalItems={rows.length} pageSize={PAGE_SIZE} onPageChange={setPage} label="صفحه‌بندی کلمات کلیدی"/>
    </section>
  </>;
}
