...
function OpportunityTable({ rows }: { rows: SelectedOpportunity[] }) {
  return <section className="analysis-table-card selected-opportunities">
    ...
    {rows.length ? <div className="analysis-table-scroll"><table className="analysis-table"><thead><tr><th>کلمه کلیدی</th><th>صفحه</th><th>دلیل</th><th>نمایش</th><th>رتبه</th><th>امتیاز</th></tr></thead><tbody>{rows.map((row) => <tr key={`${row.query}-${row.page}`}><td title={row.query}>{row.query}</td><td title={row.page}>{row.page}</td><td>{row.reason === 'low-ctr' ? 'CTR پایین' : 'نزدیک صفحه اول'}</td><td>{faNumber.format(row.impressions)}</td><td>{faDecimal.format(row.position)}</td><td><b className="opportunity-score">{faNumber.format(row.score)}</b></td></tr>)}</tbody></table></div> : <div className="analysis-empty">برای ساخت این فهرست، پس از اعمال migration یک‌بار داده‌ها را دوباره همگام‌سازی کنید.</div>}
  </section>;
}
