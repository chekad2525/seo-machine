...
The independent keyword tracker accepts `.xlsx`, UTF-8 `.csv`, and pasted lists with a `keyword` column and an optional `target_url` column (or treats the first column as keywords when there is no header). Country, language, location, and the provider-supported device are saved per project. Configure `SERPER_API_KEY`, or the DataForSEO credentials, for live Google rank checks. On a long-running API worker, `SERP_SYNC_ENABLED=true` scans due keywords throughout the day. On Vercel, keep it false and configure `CRON_SECRET`; the daily cron in `apps/api/vercel.json` triggers rank collection at 03:00 UTC. Stored snapshots power rank history, filtered CSV exports, and deterministic SEO actions; Search Console metrics are not used in this report.
