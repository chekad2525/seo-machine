...
function scheduledApprovalError(
  ...
  return new AppError(
    "VALIDATION_ERROR",
    `Adding these keywords would make each ${scheduleInterval} scheduled check cost a worst-case SerpApi estimate of ${estimate.costCredits} credits (~$${estimate.costUsd.toFixed(4)} per check; ~${estimate.monthlyCostCredits} credits/month). Call estimate_rank_tracker_cost with additionalKeywordCount, show the recurring estimate to the user, then retry with maxEstimatedScheduledCheckCredits set to the approved per-check estimate. Actual usage can be lower because pagination stops when the tracked domain is found.`,
  );
}
