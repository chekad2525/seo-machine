    ...
    >
      <div className="flex items-stretch rounded-lg border border-base-300 bg-base-100">
        <button
          ref={triggerRef}
          type="button"
          aria-label="تغییر پروژه"
          aria-expanded={open}
          aria-haspopup="listbox"
          onClick={() => (open ? closePanel() : openPanel())}
          onKeyDown={handleTriggerKeyDown}
          className="flex min-w-0 flex-1 items-center justify-between gap-2 rounded-r-lg px-3 py-1.5 text-right transition-colors hover:bg-base-200"
        >
          <span className="flex min-w-0 flex-col">
            <span className="truncate text-sm font-medium text-base-content">
              {activeProject?.name ?? "انتخاب پروژه"}
            </span>
        ...
        {activeProject ? (
          <Link
            ...
            className="flex shrink-0 items-center justify-center rounded-l-lg border-r border-base-300 px-2.5 text-base-content/60 transition-colors hover:bg-base-200 hover:text-base-content"
          >
              ...
              {filteredProjects.length === 0 ? (
                <li className="menu-disabled">
                  <span className="text-base-content/50">
                    پروژه‌ای برای «{query.trim()}» پیدا نشد
                  </span>
                </li>
              ) : null}
            </ul>
