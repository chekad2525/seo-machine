    ...
    onError: (error) =>
      toast.error(
        getStandardErrorMessage(
          error,
          "تنظیم شما ذخیره نشد. دوباره تلاش کنید.",
        ),
      ),
  });
  ...
  return (
    ...
    >
      <header className="border-b border-base-300 px-5 py-5 sm:px-6">
        <h2 className="text-lg font-semibold">فضای کاری خود را راه‌اندازی کنید</h2>
        <p className="mt-1 text-sm text-base-content/65">
          وب‌سایت را اضافه کنید، ابزارها را وصل کنید و هم‌تیمی‌ها را دعوت کنید.
        </p>
      </header>
      {remaining.map((item) => {
        ...
        return (
          <div key={item.id} className="border-b border-base-300">
            <button
              ...
              className={`flex w-full items-center gap-3 px-5 py-4 text-right sm:px-6 ${active ? "bg-primary/5" : "hover:bg-base-200/50"}`}
            >
      ...
      {deferred.length > 0 && (
        <details className="group border-t border-base-300">
          <summary className="flex cursor-pointer list-none items-center gap-2 px-5 py-4 text-sm text-base-content/65 sm:px-6 [&::-webkit-details-marker]:hidden">
            ...
            {deferred.length} مورد برای بعد ذخیره شده
          </summary>
          <ul className="space-y-1 px-5 pb-4 sm:px-6">
            {deferred.map((item) => (
              ...
              >
                ...
                <button
                  type="button"
                  aria-label={`بازگرداندن ${item.label}`}
                ...
                >
                  <RotateCcw className="size-3.5" /> بازگرداندن
                </button>
              </li>
            ))}
          </ul>
      ...
      {completed.length > 0 && (
        <details className="group border-t border-base-300">
          <summary className="flex cursor-pointer list-none items-center gap-2 px-5 py-4 text-sm sm:px-6 [&::-webkit-details-marker]:hidden">
            ...
            {completed.length} مورد تکمیل شده
            <ChevronRight className="mr-auto size-4 text-base-content/60 transition-transform group-open:rotate-90" />
          </summary>
