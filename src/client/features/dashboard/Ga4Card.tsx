  ...
  return (
    <div className="rounded-md border border-base-300 bg-base-100 px-3 py-2 shadow-sm">
      ...
      <p className="text-sm font-medium tabular-nums">
        {formatCount(payload[0].value)} نشست
      </p>
    </div>
  );
}
...
export function Ga4Card({
  ...
  return (
    <CardShell
      title="ترافیک ارگانیک"
      stamp="گوگل آنالیتیکس · ۲۸ روز گذشته"
      action={
        <Link
    ...
    >
      {reportQuery.isPending ? (
        ...
        <p className="text-sm text-base-content/60">
          داده‌های گوگل آنالیتیکس بارگذاری نشد. کمی بعد دوباره تلاش کنید.
        </p>
      ) : report?.connected ? (
        ...
        !report.totals.sessions ? (
          <p className="text-sm text-base-content/60">
            هنوز ترافیک جست‌وجوی ارگانیک در ۲۸ روز گذشته ثبت نشده است.
          </p>
        ) : (
