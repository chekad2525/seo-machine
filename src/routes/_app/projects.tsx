        ...
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-sm text-base-content/60">
              هر پروژه سرچ کنسول، ردیابی رتبه و ممیزی‌های مخصوص خود را دارد.
            </p>
          </div>
          <button
            type="button"
                ...
                >
                  <span className="flex min-w-0 flex-col">
                    ...
                    <span className="truncate text-xs text-base-content/50">
                      {project.domain ?? "دامنه‌ای ثبت نشده است"}
                    </span>
                  </span>
...
function ArchivedProjects() {
  ...
  const restoreMutation = useMutation({
    ...
    onSuccess: async () => {
      ...
      toast.success("پروژه بازیابی شد");
    },
    onError: (error) =>
      toast.error(getStandardErrorMessage(error, "بازیابی پروژه ناموفق بود")),
  });
  ...
  return (
    <section className="space-y-3">
      <h2 className="text-sm font-medium text-base-content/50">بایگانی‌شده</h2>
      <ul className="divide-y divide-base-300 overflow-hidden rounded-lg border border-base-300">
